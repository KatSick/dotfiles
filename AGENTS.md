...
- Sol's `config.json` is seeded by `home.activation.solConfig` in `home.nix`, not `mkOutOfStoreSymlink`ed like every other config here. Sol rewrites the file by rename, which destroys a symlink and then silently reverts the user's settings on the next switch. Do not "restore consistency" by linking it - the reasoning is in the comment above that activation entry.
