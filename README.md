...
- The `cc` and `co` shell aliases in `home.nix` are high-agency shortcuts: `claude` and `codex`.
...
Raycast is not covered by this. Its settings live in a database and a plist under `~/Library`, and its only portable format is an encrypted `.rayconfig` export that also carries snippets and quicklinks - neither is something a symlink or a public repo should hold.
(`~/.config/raycast` exists but is just downloaded extension bundles.)
So the cask is declared in `configuration.nix`, its settings are left to Raycast's own Cloud Sync or scheduled backup, and `home.nix` only frees Cmd+Space for it by clearing the Spotlight shortcut.
